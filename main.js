...
window.addEventListener("load", () => {
	...
	el("Bm").addEventListener("click", function () {
		var prompt = window.prompt("Paste in a raw Bingo Card. Format from the Hypixel API.");
		if(!prompt) {
			console.log("Aborted manual card read: no user input");
			return;
		}
		var json, lock = false;
		try {
			json = JSON.parse(prompt);
			if(!json.success) throw new Error("Success value is false or does not exist");
			if(!json.goals) throw new Error("Goals value is false or does not exist");
		} catch (e) {
			lock = true;
			console.log("Aborted manual card read: user input is not valid Hypixel API raw data, "+e);
		}
		if(lock) {
			return;
		}
